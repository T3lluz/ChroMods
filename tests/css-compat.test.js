...
test("all stylesheet modules exist", () => {
  ...
  assert.ok(fs.existsSync(path.join(stylesDir, "feed-layout.css")));
...
test("theater glass comments includes webkit backdrop filter", () => {
...
test("popup defines theater subsettings", () => {
  const js = read("popup/popup.js");
  assert.match(js, /hoverComments/);
  assert.match(js, /glassComments/);
