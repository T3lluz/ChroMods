...
export function buildCloudAgentPrompt({ issue, classification }) {
  ...
  return `You are working in the ChroMods Chrome MV3 extension repo (T3lluz/ChroMods).
...
- Port these non-transparency features:
${features || "- (all non-transparency CSS in that file)"}

## Required playbook (also in AGENTS.md)
1. Split upstream CSS into focused files under \`styles/<site-id>/\`.
2. Chromium-adapt: flatten nested \`&\` selectors, drop \`@-moz-document\` and Firefox-only \`scrollbar-width\`, skip ALL transparency / transparent page-background features.
3. Wire the feature ids in:
   - \`FEATURES\` + \`FEATURE_SITE\` (or prefix helper) in \`scripts/content-script.js\`
   - defaults in \`scripts/background.js\` and \`scripts/content-script.js\`
   - \`FEATURE_META\` in \`scripts/popup.js\` with \`site: "<site-id>"\`
   - \`SITE_META\` in \`scripts/sites.js\` if this is a new website
   - icon in \`scripts/icons.js\` and \`docs/sites/<site-id>.svg\`
   - \`manifest.json\`: theming content_scripts matches AND web_accessible_resources matches for the host
4. If tests currently assert this site is absent (for example \`doesNotMatch\` / \`site-twitch\`), update those tests to expect the new site. Add coverage in \`tests/css-compat.test.js\`.
5. Keep MIT attribution; CSS is derived from my-internet.
6. Run \`npm test\` and \`npm run readme\`. Do not open a PR if tests fail.
7. PR title like \`Add ${host} theming mods\`. PR body must include \`Fixes #${issue.number}\`.
